...
-- === REPAIR V25: BUDGETING & ALLOCATIONS ===

-- 1. ADD COLUMNS TO BUDGETS
ALTER TABLE budgets ADD COLUMN IF NOT EXISTS is_active boolean DEFAULT false;
...
-- 3. CREATE INVOICE_ALLOCATIONS TABLE
...
-- 5. RLS POLICIES (Superusers & Team)
...
USING (
    EXISTS (
        ...
        AND public.can_manage_invoice(budgets.project_id) -- Reuse function: implies member or owner
    )
);
...
-- INVOICE ALLOCATIONS
DROP POLICY IF EXISTS "Allocations Read" ON invoice_allocations;
CREATE POLICY "Allocations Read" ON invoice_allocations FOR SELECT TO authenticated
USING (
    EXISTS (
        SELECT 1 FROM invoices 
        WHERE invoices.id = invoice_allocations.invoice_id 
        AND public.can_manage_invoice(invoices.project_id)
    )
);
