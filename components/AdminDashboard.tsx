...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUserId }) => {
  ...
  const getRepairSql = () => {
    return `
-- REPAIR SCRIPT v7.0 (Fix Missing Columns & Budget Table)

-- 1. DROP OLD FUNCTION FIRST to avoid conflict
...
-- FORCE ADD COLUMNS TO PROFILES/INVITES
do $$ begin
...
-- Budgets: Access via Project ownership
drop policy if exists "Manage own budgets" on budgets;
create policy "Manage own budgets" on budgets for all to authenticated
using ( exists ( select 1 from projects where id = budgets.project_id and (created_by = auth.uid() OR lower(auth.jwt() ->> 'email') = 'tadekus@gmail.com') ) );

-- Profiles: I am master OR I invited them OR it is me
...
-- Invites: I am master OR I sent them OR it is for me
